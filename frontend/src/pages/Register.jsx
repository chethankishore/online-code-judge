...
//           <p style={styles.subtitle}>Start your coding journey</p>
//         </div>
...
//           </div>
//           <input style={styles.input} name="username" placeholder="Username" value={form.username} onChange={onChange} required />
//           <input style={styles.input} name="email" type="email" placeholder="Email" value={form.email} onChange={onChange} required />
//           <input style={styles.input} name="password" type="password" placeholder="Password (min 6 chars)" value={form.password} onChange={onChange} required />
...
//   divider: { textAlign: 'center', color: '#333', margin: '24px 0', fontSize: '13px' },
//   oauthRow: { display: 'flex', gap: '12px' },
//   oauthBtn: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: '8px', padding: '11px', color: '#fff', fontSize: '14px', textDecoration: 'none', fontFamily: "'Fira Code', monospace" },
...
// --- Blood Canvas & Cursed Lines (same as in Login/Landing) ---
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await register(form.username, form.email, form.password, form.firstName, form.lastName);
      toast.success('Your oath is sealed. Welcome, sorcerer.', { style: { background: '#1a0000', color: '#00ff88' } });
      navigate('/dashboard');
    } catch (err) {
      toast.error(err.response?.data?.message || 'Failed to forge the oath', { style: { background: '#1a0000', color: '#ff4444' } });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div style={{
          ...
          <div style={{
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: 'block', color: '#aa4444', fontSize: 11, letterSpacing: '0.3em', marginBottom: 6 }}>DOMAIN EMAIL</label>
                <input
                  name="email"
                  type="email"
                  placeholder="sorcerer@jujutsu.com"
                  value={form.email}
                  onChange={onChange}
                  required
                  style={{
              ...
              >
                {loading ? 'FORGING OATH...' : 'ᚠ BECOME A SORCERER ᚠ'}
              </button>
            </form>
